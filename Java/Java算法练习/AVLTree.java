...
public class AVLTree{
   ...
   static Boolean bool_search(int data,AvlNode T){
       if(data=T.data)
            return true;
        else if(data>T.data)
            return bool_search(data, T.rchild);
        else if(data<T.data)
            return bool_search(data, T.lchild);
   ...
   public static void main(String[] args) {
      ...
      //  AVLTree.insert(5,T);
        ...
        while(scn.hasNext()){
            n=scn.nextInt();
            T=AVLTree.insert(n, T);
        }
        int test=scn.nextInt();
        scn.close();
        System.out.println(bool_search(test,T));
   }
}
